(() => {
 ...
 $('player-message-form').onsubmit=event=>{event.preventDefault();const text=$('player-message-text').value.trim();if(!text)return;run($('player-message-send'),uid=>db.collection('tinkleInboxes').doc(uid).collection('messages').add({text,by:auth.currentUser.uid,createdAt:firebase.firestore.FieldValue.serverTimestamp(),read:false}),'Private message sent.');};
